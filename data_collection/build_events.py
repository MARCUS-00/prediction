# ============================================================
# data_collection/build_events.py  (FIXED v12)
#
# FIX 1: START_DATE enforced to 2020-01-01.
# FIX 2: REMOVED _build_fallback_events() entirely.
#   Fallback generated SYNTHETIC earnings/board-meeting dates
#   based on deterministic schedules — violating the "no fake data"
#   constraint. If the NSE API returns < 5 events, we now accept
#   whatever we have (macro events are real) rather than fabricating.
# FIX 3: event_flag=0 / event_score_max=0 for non-event days.
#   This was already correct in the original, kept as-is.
# ============================================================
...
# Real RBI policy dates (filtered to 2020-2025 in code below)
...
def fetch_corporate_events(symbol, start_date, end_date):
    """
    FIX 2: Removed fallback logic entirely.
    If NSE API returns < 5 events, we simply use what we have.
    Macro events (RBI, CPI, etc.) provide real signal for all stocks.
    """
    events      = []
    ...
    # FIX 2: No fallback. Accept real events only.
...
def main():
    ...
    print("=" * 60)
    print(f"  BUILD EVENTS DATASET (FIXED v12 — no synthetic fallback)")
    ...
    # M&M validation
    mm = df[df["symbol"] == "M&M"]
    mm_events = mm[mm["is_event"] == 1]
    print(f"\n  M&M event days: {len(mm_events)} (out of {len(mm)} trading days)")
    if len(mm_events) == 0:
        print("  [WARN] M&M has no corporate events — only macro events expected for this stock")
    print("=" * 60)

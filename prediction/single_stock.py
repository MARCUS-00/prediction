...
def predict_single(symbol: str, df: pd.DataFrame = None) -> dict:
    if df is None or df.empty:
        if not os.path.exists(MERGED_CSV):
            return {"error": f"merged_final.csv not found. Run: python features/merge_features.py"}
        try: df = pd.read_csv(MERGED_CSV)
        except Exception as e: return {"error": f"Cannot load dataset: {e}"}

    sdf = df[df["Stock"] == symbol].sort_values("Date").reset_index(drop=True)
    if sdf.empty:
        return {"error": f"No data for '{symbol}'. Check symbol is in merged_final.csv"}
